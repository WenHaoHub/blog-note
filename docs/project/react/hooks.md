...
1. 全等的比较 react 默认由于堆地址不同 所以全等判断为 false
...
父组件重新渲染时，子组件也会重新渲染，无论子组件的 props 是否真的发生变化。React 在决定是否重新渲染组件时，使用的是“浅比较”（Shallow Comparison），即只比较对象的引用是否相同，而不比较对象内部的值是否相同。

但是重新渲染组件的场景，会重新创建一个相同的对象，但是地址不同所以浅比较是 false
